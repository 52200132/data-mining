...
# RULE-BASED
TOPIC_KEYWORDS = {
    "Thể thao": ["bóng_đá", "cầu_thủ", "giải_đấu", "trận_đấu", "vận_động_viên", "huy_chương", "thể_thao", "bàn_thắng", "câu_lạc_bộ", "hlv", "vô_địch", "tuyển", "huấn_luyện_viên", "sân_vận_động"],
    "Kinh doanh": ["doanh_nghiệp", "thị_trường", "chứng_khoán", "tài_chính", "ngân_hàng", "lợi_nhuận", "đầu_tư", "xuất_khẩu", "gdp", "cổ_phiếu", "doanh_thu", "kinh_doanh", "bất_động_sản"],
    "Pháp luật": ["tòa_án", "bị_cáo", "xét_xử", "tội_phạm", "hình_sự", "điều_tra", "bắt_giữ", "luật", "phiên_tòa", "kết_án", "công_an", "bản_án", "vi_phạm", "khởi_tố"],
    "Công nghệ": ["công_nghệ", "phần_mềm", "trí_tuệ_nhân_tạo", "smartphone", "ai", "internet", "robot", "ứng_dụng", "kỹ_thuật_số", "chip", "điện_thoại", "máy_tính"],
    "Giải trí": ["nghệ_sĩ", "âm_nhạc", "phim", "ca_sĩ", "diễn_viên", "showbiz", "nhạc", "điện_ảnh", "hoa_hậu", "sao", "concert", "album"],
    "Chính trị - Xã hội": ["chính_phủ", "bộ_trưởng", "nghị_quyết", "chính_sách", "xã_hội", "dân_sinh", "giao_thông", "quy_định", "nhà_nước", "quốc_hội", "thủ_tướng"],
    "Sức khỏe": ["bệnh_viện", "bác_sĩ", "bệnh_nhân", "sức_khỏe", "y_tế", "vaccine", "điều_trị", "thuốc", "dịch_bệnh", "phẫu_thuật", "ung_thư", "dinh_dưỡng"],
    "Du lịch": ["du_lịch", "khách_sạn", "điểm_đến", "tour", "resort", "biển", "di_tích", "tham_quan", "danh_lam", "khách_du_lịch", "phong_cảnh", "ẩm_thực"],
    "Giáo dục": ["học_sinh", "sinh_viên", "trường", "giáo_viên", "đại_học", "thi_cử", "giáo_dục", "chương_trình", "tuyển_sinh", "bộ_giáo_dục", "học_bổng", "kỳ_thi"],
    "Khoa học": ["nghiên_cứu", "khoa_học", "phát_hiện", "vũ_trụ", "gen", "tế_bào", "thí_nghiệm", "nhà_khoa_học", "dna", "hành_tinh", "vật_lý", "hóa_học"],
}
...
def main():
    ...
    with col1:
        title_input = st.text_input(
            "Tiêu đề bài báo",
            placeholder="VD: Arsenal liệu có cú ăn 2 mùa này?"
        )
    with col2:
        st.write("")  # spacer
    ...
    if st.button("🔍 Phân loại", type="primary", use_container_width=True):
        ...
        if method in ["Rule-based (từ khóa)", "So sánh cả hai"]:
            ...
            with st.container():
                ...
                if rb_pred != "Không xác định":
                    ...
                    st.warning(" Không xác định được chủ đề (văn bản quá ngắn hoặc không có từ khóa)")
                ...
                with st.expander("Xem điểm từng chủ đề (rule-based)"):
                    ...
                    for topic, score in sorted_scores:
                        if score > 0:
                            bar = "█" * score
                            st.text(f"{TOPIC_ICONS.get(topic,'')} {topic:25s} {bar} ({score})")
            ...
            if model_loaded:
                ...
                with st.container():
                    ...
                    with st.expander("Xem xác suất tất cả chủ đề"):
                        ...
                        topics_list  = [f"{TOPIC_ICONS.get(t,'')} {t}" for t, _ in sorted_proba]
                ...
                st.info(" Model chưa được train. Chạy `poetry run classify/train_model.py` trước.")
        ...
        if method == "So sánh cả hai" and model_loaded:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.info(f"**Rule-based:** {TOPIC_ICONS.get(rb_pred_final,'')} {rb_pred_final}")
            with c2:
                conf = lr_proba_final[lr_pred_final] * 100
                st.success(f"**LR Model:** {TOPIC_ICONS.get(lr_pred_final,'')} {lr_pred_final} ({conf:.0f}%)")
